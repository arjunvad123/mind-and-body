    ...
    def train(self, dataset: StateStreamDataset, n_epochs: int = 50,
        ...
        for epoch in range(n_epochs):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                # Convert target actions to one-hot for loss computation
                target_actions = target_data['actions']
                if target_actions.shape[-1] != pred_actions.shape[-1]:
                    act_idx = target_actions.long().squeeze(-1)
                    target_onehot = torch.zeros(
                        *act_idx.shape, pred_actions.shape[-1],
                        device=pred_actions.device
                    )
                    target_onehot.scatter_(-1, act_idx.unsqueeze(-1), 1.0)
                else:
                    target_onehot = target_actions

                # Compute losses
                action_loss = nn.MSELoss()(pred_actions, target_onehot)
