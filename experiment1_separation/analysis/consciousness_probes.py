...
class TemporalIntegrationProbe:
    ...
    def attention_analysis(self, episodes, max_episodes=20):
        """Extract and analyze attention patterns.

        Looks for a recency gradient — stronger attention to recent timesteps,
        which would indicate a "present-focused" awareness.
        """
        # Hook to capture attention weights
        attention_weights = []

        def attention_hook(module, input, output):
            # TransformerEncoderLayer doesn't directly expose attention weights
            # We'll use a proxy: gradient-based attention analysis
            pass

        # Alternative: ablation-based temporal analysis
        # Remove different portions of the sequence and measure degradation
    ...
    def ablation_analysis(self, episodes, max_episodes=20):
        ...
        for episode in episodes[:max_episodes]:
            ...
            ablated = self._zero_range(input_data, ablate_start, seq_len)
            with torch.no_grad():
                pred_a_r, _, _ = self.model(**ablated)
            recent_error = torch.norm(
                pred_a_r.squeeze(0)[-1] - input_data['actions'].squeeze(0)[-1]
            ).item()

            # Ablate distant (first 25%)
            ablate_end = int(seq_len * 0.25)
            ablated = self._zero_range(input_data, 0, ablate_end)
            with torch.no_grad():
                pred_a_d, _, _ = self.model(**ablated)
            distant_error = torch.norm(
                pred_a_d.squeeze(0)[-1] - input_data['actions'].squeeze(0)[-1]
            ).item()

            # Ablate middle (25-75%)
            mid_start = int(seq_len * 0.25)
            mid_end = int(seq_len * 0.75)
            ablated = self._zero_range(input_data, mid_start, mid_end)
            with torch.no_grad():
                pred_a_m, _, _ = self.model(**ablated)
            middle_error = torch.norm(
                pred_a_m.squeeze(0)[-1] - input_data['actions'].squeeze(0)[-1]
            ).item()
    ...
    def _zero_range(self, data, start, end):
        ...
        for k, v in data.items():
            if k == 'hidden_states':
                ...
                for h in v:
                    h_copy = h.clone()
                    h_copy[:, start:end] = 0
                    result[k].append(h_copy)
            elif isinstance(v, torch.Tensor):
                v_copy = v.clone()
                v_copy[:, start:end] = 0
                result[v_copy] = v_copy
            else:
                result[k] = v
        # Fix the bug above
        result_fixed = {}
        for k, v in data.items():
            if k == 'hidden_states':
                ...
                result_fixed[k] = new_hs
            elif isinstance(v, torch.Tensor):
                v_copy = v.clone()
                v_copy[:, start:end] = 0
                result_fixed[k] = v_copy
            else:
                result_fixed[k] = v
        return result_fixed
